...
EXPECTED_IDS <- c("typical", "redgreen", "blueyellow", "clustal", "zappo",
...
test_that("residue order is canonical", {
  expect_identical(names(aa_palette("typical")), CANONICAL_RESIDUES)
  expect_identical(names(aa_palette("shapely")), CANONICAL_RESIDUES)
})

test_that("unknown and gap defaults match the JSON", {
  pal <- aa_palette("typical", include_unknown = TRUE, include_gap = TRUE)
...
test_that("aa_colour handles lower-case, unknown, gap and stray symbols", {
  cols <- aa_colour(strsplit("ACDXacd-.?", "")[[1]], "typical")
  typical <- aa_palette("typical")
  unk <- as.character(raw$defaults$unknown_XBZJ)
  gap <- as.character(raw$defaults$gap)
  expect_equal(cols[1], unname(typical["A"]))   # A
  expect_equal(cols[4], unk)                # X -> unknown
  expect_equal(cols[5], unname(typical["A"]))   # a -> A
...
test_that("aa_scheme_info exposes label/source/vision and extras where present", {
  typical <- aa_scheme_info("typical")
  expect_equal(typical$label, raw$schemes$typical$label)
  expect_equal(typical$source, raw$schemes$typical$source)
  expect_equal(typical$vision, raw$schemes$typical$vision)
  expect_false(is.null(typical$min_deltaE))
  expect_false(is.null(typical$names))
...
test_that("aa_recommended matches the JSON recommendations", {
  rec <- aa_recommended()
  expect_equal(unname(rec["normal"]), "typical")
  expect_equal(unname(rec["red_green_cvd"]), "redgreen")
  expect_equal(unname(rec["tritan_cvd"]), "blueyellow")
})
...
test_that("ggplot2 scales build when ggplot2 is available", {
  ...
  expect_s3_class(scale_colour_aa("typical"), "Scale")
  expect_s3_class(scale_color_aa("typical"), "Scale")
...
test_that("plot_aa_palette returns the palette invisibly", {
  ...
  res <- plot_aa_palette("blueyellow")
